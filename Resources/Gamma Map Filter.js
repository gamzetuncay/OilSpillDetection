...
var imageVisParam = {"opacity":1,"bands":["VV"],"min":-25,"max":0,"gamma":1};
...
var collection2 = ee.ImageCollection('COPERNICUS/S1_GRD').filterMetadata('instrumentMode', 'equals', 'IW').
filter(ee.Filter.eq('transmitterReceiverPolarisation', 'VV')).select(['VV', 'angle']).filterBounds(caslav);

var collection = ee.ImageCollection('COPERNICUS/S1_GRD')
            .filterBounds(caslav)         
            .filter(ee.Filter.listContains('transmitterReceiverPolarisation','VV')).select(['VV', 'angle'])
