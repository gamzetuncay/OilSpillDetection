...
function FocalMedian(img, range, bandName) {
    ...
    */
    ...
    var focalMedian = band.focal_median(range,'circle','meters').rename(newBandName);
...
Map.addLayer(FocalMedian(Sar, 150, 'VV'),
             mapStretchSTD(FocalMedian(Sar, 150, 'VV'), 3, 'VV_FocalMedian'),
             'VV Focal Median');
...
Sar = Frost(Sar, 9, 'VV',  -1).select('VV_Frost');

var canny = ee.Algorithms.CannyEdgeDetector({
  image: Sar, threshold: 2, sigma: 0
});

Map.addLayer(canny, {}, 'canny');
// Perform Hough transform of the Canny result and display.
var hough = ee.Algorithms.HoughTransform(canny,256,1);
