...
for (var j = 0; j<5; j++){
    ...
    var descriptionName = 'image'.concat((j+1).toString()).concat('_');
    ...
    for(var i = 0; i<7; i++){
      ...
      var means = Sar1.map(function(image){
        image=image.clip(regions.get(1))
      ...
      if(j==2 || j==3){
        var meansList =ee.List([ means.aggregate_mean('VV_FocalMedian_mean'),
                means.aggregate_mean('VV_Frost_mean'),
                means.aggregate_mean('VV_GMAP_mean'),
                means.aggregate_mean('VV_LeeSigma_mean')])
      }
      else{
        var meansList =ee.List([ means.get('VV_FocalMedian_mean'),
                means.get('VV_Frost_mean'),
                means.get('VV_GMAP_mean'),
                means.get('VV_LeeSigma_mean')])
      }
      var size = kernelSize.toString().concat('x').concat(kernelSize.toString());
      var descriptionName2 = descriptionName.concat(size)
      ...
      for(var k = 0; k<4; k++){
        var band = bands[k];
        var descriptionName3 = descriptionName2.concat(band);
        var parameter = {bands:band,min: -40,max: -10};
        var Sar2 = Sar1.map(function(image){
        ...
        Export.image.toDrive({
          image: im,
          folder: 'Bitirme/kernel',
          description: descriptionName3,
          scale:10,
          region: re
          });
